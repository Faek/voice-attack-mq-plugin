...
namespace VoiceAttackMQPlugin
{
    ...
    //    {
    //        return new Guid("{E2D4C7FF-007A-4EE3-9892-0047FB1BB0BF}");
    //    }
    ...
    //    {
    //        ILogger logger = NullLoggerFactory.Instance.CreateLogger("VoiceAttackPluginLogger");

    //        logger.LogInformation("Initialising Twitch VoiceAttack plugin");
    ...
    {
        public void Any(QueueCommandRequest request)
        {
    ...
    public class VoiceAttackPlugin
    {
        ...
        const string C_APP_VERSION = "beta1";
        ...
        public static string VA_DisplayName()
        {
            //this is what you will want displayed in dropdowns as well as in the log file to indicate the name of your plugin
            return $"{C_APP_NAME} {C_APP_VERSION}";  
        }

        public static string VA_DisplayInfo()
        {
            return $"{C_APP_NAME} allows VoiceAttack to trigger commands by consuming messages from a message queue";  //this is just extended info that you might want to give to the user.  note that you should format this up properly.
        }

        public static Guid VA_Id()
        {
            return new Guid("{E51D41B3-3D1F-4E27-8976-874BA8DF37B5}");  //this id must be generated by YOU... it must be unique so VoiceAttack can identify and use the plugin
        }
        ...
        public static void VA_Init1(dynamic vaProxy)
        {
            ...
            //_messageQueue = new RabbitMqServer();
            //_messageQueue.RegisterHandler<VAExecCommand>(m => {
            //    VAExecCommand request = m.GetBody();
            //    Console.WriteLine($"Hello, {request.CommandName}!");
            //    return null;
            //});

            //_appHost = new BasicAppHost(typeof(VoiceAttackPlugin).Assembly)
            //{
            ...
            //            {
            //                setStatus(vaProxy, VAMQStatus.Error, new Exception($"\"{request.CommandName}\" command not found"));
            //                return null;
            //            }
            ...
            //            {
            //                setStatus(vaProxy, VAMQStatus.Error, new Exception($"\"{request.CommandName}\" is already running, skipped running command."));
            //                return null;
            //            }
        ...
        //private static void setException(ref dynamic vaProxy, Exception exception = null)
        //{
        //    if (exception != null) 
        //        Logger.Warn("VAMQ error: " + exception.ToString());

        //    vaProxy.SetText("VAMQ exception", exception?.ToString());
        //}
        private static void setStatus(ref dynamic vaProxy, VAMQStatus status, Exception exception = null)
        {
            vaProxy.SetText("VAMQ Status", status);
            if (status == VAMQStatus.Operational)
            {
                vaProxy.SetText("VAMQ exception", null);
            }
            else
            {
                Logger.Warn("VAMQ error: " + (exception == null ? "<null>" : exception.ToString()));
                vaProxy.SetText("VAMQ exception", exception?.ToString());
            }
        }
